...
// Komponen untuk latar belakang digital
const DigitalTechBackground = () => {
    return (
        <div className="absolute top-0 left-0 w-full h-full z-0 overflow-hidden">
            {/* Pola grid halus */}
            <div className="absolute inset-0 z-[-2] bg-[linear-gradient(to_right,hsl(var(--border)/0.1)_1px,transparent_1px),linear-gradient(to_bottom,hsl(var(--border)/0.1)_1px,transparent_1px)] bg-[size:36px_36px]"></div>
            {/* Efek glow radial dari atas */}
            <div className="absolute inset-0 z-[-1] bg-[radial-gradient(ellipse_50%_50%_at_50%_-20%,hsl(var(--secondary)/0.3),transparent)]"></div>
        </div>
    )
}

// Komponen Kartu Fitur dengan animasi baru
const FeatureCard = ({ icon, title, description, index }: { icon: React.ElementType, title: string, description: string, index: number }) => (
  <motion.div 
    className="bg-card/30 backdrop-blur-sm border border-border/50 rounded-xl p-6 text-center h-full group transition-all duration-300 hover:border-secondary hover:bg-secondary/10"
    initial={{ opacity: 0, y: 50 }}
    whileInView={{ opacity: 1, y: 0 }}
    viewport={{ once: true, amount: 0.5 }}
    transition={{ delay: index * 0.15, duration: 0.6, ease: "easeOut" }}
  >
    <div className="mx-auto mb-4 inline-block rounded-full bg-primary p-4 border border-border group-hover:border-secondary transition-colors duration-300">
      {React.createElement(icon, { className: "h-8 w-8 text-secondary group-hover:text-accent2 transition-colors duration-300" })}
    </div>
